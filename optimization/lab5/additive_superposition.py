...
print("Метод аддитивной свертки")
...
f1_star, f2_star, f3_star = f(mins[0], 0), f(mins[1], 1), f(mins[2], 2)

print("\nМинимумы критериев:")
print(f"  f₁* = {f1_star:.2f} в точке ({mins[0][0]:.1f}, {mins[0][1]:.1f})")
print(f"  f₂* = {f2_star:.2f} в точке ({mins[1][0]:.1f}, {mins[1][1]:.1f})")
print(f"  f₃* = {f3_star:.2f} в точке ({mins[2][0]:.1f}, {mins[2][1]:.1f})")
...
for a1 in np.linspace(0, 1, 10):
    for a2 in np.linspace(0, 1, 10):
        ...
        if result.success:
            ...
            pareto_weights.append((a1, a2, a3))
...
plt.xlabel('x₁')
plt.ylabel('x₂')
plt.title('Множество Парето в пространстве переменных')
